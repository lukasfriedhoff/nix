...
in
{
  ...
  systemd.services.srv7-bootstrap-password = {
    description = "Apply srv7 bootstrap password from SOPS secret";
    ...
    script = ''
      set -euo pipefail
      secret="${config.sops.secrets."srv7-bootstrap-password".path}"
      if [ ! -s "$secret" ]; then
        exit 0
      fi
      ...
      if [ -z "$password" ]; then
        echo "srv7 bootstrap password secret is empty" >&2
        exit 1
      fi

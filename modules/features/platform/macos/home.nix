...
}:
{
  ...
  home.packages = lib.mkIf pkgs.stdenv.isDarwin (
    ...
    [
      ...
      rustc
      cargo
      # Single python provider for the Mac: one withPackages env instead of
      # a bare python3 + pip (the MLX runtime lives in its own uv venv, see
      # lukasf.mlxLm). A bare python3 and a python withPackages env both
